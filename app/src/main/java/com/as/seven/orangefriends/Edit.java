...
//        setResult(RESULT_OK, it);
        ...
        ArrayList<HashMap<String, String>> TableSqlResult = new ArrayList<>();
        // 更新
        if (null!=noteId && (!(noteId.isEmpty()))) {
            if (null != edt.getText() && !("".equalsIgnoreCase(edt.getText().toString()))) {
                ContentValues contentValues = new ContentValues();
                contentValues.clear();
                contentValues.put("tag", tag);
                contentValues.put("mainText", edt.getText().toString());
                contentValues.put("alarm", alarm);
                contentValues.put("updatetime", new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date()));
                mDBManager.Update("t_of_notelist", contentValues, selection, args);
            }
        } else {
            // 新建
            if (null != edt.getText() && !("".equalsIgnoreCase(edt.getText().toString()))) {
                ...
                contentValues.put("updatetime", new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date()));
                mDBManager.Insert("t_of_notelist", contentValues);
            }
        }
        finish();
    }
